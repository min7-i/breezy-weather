...
class StatementManager @Inject constructor(
    ...
    private fun getPermissionKey(permission: String): String =
        permission.replace(".", "_").lowercase()
    ...
    fun isPermissionDenied(permission: String): Boolean =
        config.getBoolean(getPermissionKey(permission) + "_denied", false) // TODO: add "_denied" to string?

    fun setPermissionDenied(permission: String) {
        config.edit().putBoolean(getPermissionKey(permission) + "_denied", true).apply()
    }
